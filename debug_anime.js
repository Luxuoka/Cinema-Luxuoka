...
async function run() {
    console.log("Searching for 'Gintama' on Jikan...");
    const animes = await searchAnime('Gintama');

    for (const anime of animes) {
        console.log(`\nChecking Anime: ${anime.title} (MAL ID: ${anime.mal_id})`);
        ...
        if (mapping) {
            ...
            console.log(`Embed URL: https://vidsrc.to/embed/${mapping.type === 'movie' ? 'movie' : 'tv'}/${mapping.id}`);
        } else {
